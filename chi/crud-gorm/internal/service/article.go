...
type ArticleRepository interface {
	...
	Filter(ctx context.Context, title, content string) ([]domain.Article, error)
}
...
func (s *ArticleService) FilterArticles(ctx context.Context, title, content string) ([]domain.Article, error) {
	articles, err := s.repo.Filter(ctx, title, content)
	if err != nil {
		return nil, fmt.Errorf("s.repo.FilterArticles -> %w", err)
	}
